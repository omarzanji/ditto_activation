...
for i in range(10):
    files = os.listdir(f'elvenlabs_samples/session{i+1}/')
    for ndx, file in enumerate(files):
        stamp = int(time.time())+ndx+1
        os.system(
            f'ffmpeg -y -i "elvenlabs_samples/session{i+1}/{file}" -ar 16000 "raw_data/heyditto-{stamp}-{file}.wav"')

for i in range(7):
    files = os.listdir(f'elvenlabs_samples/session{i+1}-background/')
    num_files = len(files)
    ...
    for ndx, file in enumerate(files):
        ...
        stamp = int(time.time())+ndx+1
        y, s = librosa.load(
            f'elvenlabs_samples/session{i+1}-background/{file}', sr=16000, mono=True)
        seconds = y.size / 16000
        ...
        for x in range(0, int(y.size), chunk_size):  # iterate through each second
            sf.write(
                f'raw_data/background_{file}_{x}_{stamp}.wav', y[i:i+chunk_size+1], 16000)
